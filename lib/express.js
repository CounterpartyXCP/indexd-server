...
module.exports = function initialize (adapter, opts) {
  ...
  let searchRange = (fn, scId, cb) => fn({scId, heightRange: [0, 2500000]}, 3000000, cb)
  ...
  router.get('/a/:address/utxos', (req, res) => {
    ...
    parallel({
      ...
      utxos: (cb) => searchRange(adapter.utxosByScriptRange, scId, cb)
    }, (err, results) => {
      if (err) return respond(req, res, err)
...
let inspect = require('util').inspect
  router.get('/a/:address/txos', (req, res) => {
    ...
    parallel({
      tip: (cb) => adapter.tips(cb),
      txos: (cb) => searchRange(adapter.transactionIdsByScriptRange, scId, cb)
    }, (err, results) => {
      if (err) return respond(req, res, err)
  ...
  router.get('/a/:address/txs', (req, res) => {
    ...
    searchRange(adapter.transactionIdsByScriptRange, scId, (err, txIdSet) => {
      if (err) return respond(req, res, err)
  ...
  router.get('/a/:address/balance', (req, res) => {
    ...
    searchRange(adapter.utxosByScriptRange, scId, (err, results) => {
      if (err) return respond(req, res, err)
